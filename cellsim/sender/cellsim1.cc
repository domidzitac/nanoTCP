...
bool RED (double size)
{
  ...
  if (size <= min_TH)
    ...
    prob = min_prob + ((max_prob-min_prob)/(max_TH-min_TH))*(size-min_TH);
...
void DelayQueue::write( const string & packet )
{
  ...
  // Yasir
  //if (RED(_queued_bytes+packet.size())) {
  if ((_queued_bytes+packet.size()) > 10000000) {
    _packets_dropped++;
          fprintf(stderr, "%s, packet size %lu dropped due to tail drop at the buffer at %lu \n", _name.c_str(), packet.size(), (_queued_bytes+packet.size()));
  }
  else {
    uint64_t now( timestamp() );
